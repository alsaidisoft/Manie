...
{
    [MetadataType(typeof(RA42_AIR_CREW_PASS_DTLMetadata))]
    public partial class RA42_AIR_CREW_PASS_DTL
    {
        RA42_SECURITY_CLEARANCEEntities db = new RA42_SECURITY_CLEARANCEEntities();
        private int ACCESS_TYPE = 10;
        public static implicit operator RA42_AIR_CREW_PASS_DTL(HttpPostedFileBase v)
        {
            throw new NotImplementedException();
        }

        public int countCommennts(int air_crew_pass_code)
        {
           
            var v = db.RA42_COMMENTS_MST.Where(a => a.PASS_ROW_CODE == air_crew_pass_code && a.ACCESS_TYPE_CODE == ACCESS_TYPE).Count();
            return v;
        }
    ...
    public class RA42_AIR_CREW_PASS_DTLMetadata
    {
        public int AIR_CREW_PASS_CODE { get; set; }
